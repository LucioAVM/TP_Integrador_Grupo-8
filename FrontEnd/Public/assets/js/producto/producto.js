...
import { openProductoDetalleModal, initProductoDetalleModalRoot } from '../components/productoDetalleModal.js';
...
export function initProducto() {
  ...
  getFiltros().then(({ categorias, tipos }) => {
    ...
    loadPage(1).then(() => abrirDetalleDesdeQuery());
  });
}
...
async function abrirDetalleDesdeQuery() {
  ...
  const enPagina = productosGlobal.find(p => String(p.id) === String(detalleId));
  if (enPagina) {
    openProductoDetalleModal(enPagina);
    return;
  }

  try {
    const res = await fetch(`/api/productos/${encodeURIComponent(detalleId)}`);
    if (!res.ok) return;
    const producto = await res.json();
    openProductoDetalleModal(producto);
  } catch (err) {
    console.error('No se pudo abrir detalle desde URL:', err);
  }
}
...
function renderProductos(productos) {
  ...
  container.querySelectorAll('.btn-detalle').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = Number(btn.getAttribute('data-id'));
      const producto = productosGlobal.find(p => p.id === id);
      if (producto) openProductoDetalleModal(producto);
    });
  });
  animarProductos();
