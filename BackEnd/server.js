...
import VentaProducto from './src/Models/venta_productos.js';
...
app.get('/dashboard', requireAdmin, async (req, res) => {
  ...
  const mensaje = req.query.mensaje || null; // Mensaje opcional
  const error = req.query.error || null; // Error opcional
...
app.get('/ventas', requireAdmin, async (req, res) => {
  try {
    ...
    console.log('Ventas obtenidas:', ventas); // Verificar los datos en la consola
...
app.post('/api/ventas', async (req, res) => {
  try {
    ...
    // Crear la venta (sin enviar la fecha)
    const venta = await Venta.create({ nombre_usuario, total });

    // Crear los productos relacionados
    for (const p of productos) {
      await VentaProducto.create({
        venta_id: venta.id,
        producto_id: p.producto_id,
